...
static event_exec_argvtype_t __str2argvtype(_Nt_array_ptr<const char> str) {
    if (!str)
        str = "(BAD VALUE)";
...
static int event_exec_emit(void* state, _Ptr<event_t> event) {
...
int event_get_exec(_Ptr<event_registration_t> er, _Ptr<config_options_t> options) {
    event_exec_t *self = calloc(1, sizeof(event_exec_t));
    _Ptr<config_options_t> cur = NULL;
    size_t extra_argc = 0;
