...
static listensocket_t * listensocket_container_get_by_id(_Ptr<listensocket_container_t> self, const char *id : itype(_Nt_array_ptr<const char> ) );
static int listensocket_container_configure__unlocked(_Ptr<listensocket_container_t> self, _Ptr<const ice_config_t> config);
...
static _Ptr<const char> __string_default(const char *str : itype(_Ptr<const char> ) , _Ptr<const char> def)
{
...
int listensocket_container_configure(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) , _Ptr<const ice_config_t> config)
{
    int ret;
...
{
    listensocket_t **n;
    _Ptr<listensocket_t*> match = NULL;
...
int listensocket_container_configure_and_setup(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) , _Ptr<const ice_config_t> config)
{
...
int listensocket_container_setup(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) )
{
    int ret;
...
_Ptr<connection_t> listensocket_container_accept(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) , int timeout)
{
    listensocket_t *ls;
...
int listensocket_container_set_sockcount_cb(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) , _Ptr<void (size_t , void* )> cb, void* userdata)
{
    if (!self)
        return -1;
...
ssize_t listensocket_container_sockcount(listensocket_container_t *self : itype(_Ptr<listensocket_container_t> ) )
{
    ssize_t ret;
