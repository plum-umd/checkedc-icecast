...
static int get_file_data(_Ptr<FILE> intro, client_t *client : itype(_Ptr<client_t> ) )
{
 ...
 * cause a segfault!
 */
static ssize_t __print_var(char *str : itype(_Nt_array_ptr<char> ) , size_t remaining, _Nt_array_ptr<const char> format, const char *first : itype(_Ptr<const char> ) , const http_var_t *var : itype(_Ptr<const http_var_t> ) )
{
    size_t i;
    ssize_t done = 0;
    int ret;
...
static _Nt_array_ptr<const char> __find_bitrate(const http_var_t *var : itype(_Ptr<const http_var_t> ) )
{
    size_t i;
...
static int format_prepare_headers(_Ptr<source_t> source, _Ptr<client_t> client)
{
    ...
    while (node)
    {
        ...
        if (!strcasecmp(var->name, "ice-audio-info"))
        {
            ...
            _Nt_array_ptr<const char> brfield =  NULL;
