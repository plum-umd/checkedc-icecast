...
/* Custom xslt loader */
static xmlDocPtr custom_loader(const xmlChar *URI, xmlDictPtr dict : itype(_Ptr<xmlDict> ) , int options, void *ctxt, xsltLoadType type) : itype(_Ptr<xmlDoc> ) 
{
...
static void _send_error(client_t *client : itype(_Ptr<client_t> ) , icecast_error_id_t id, int old_status) {
    if (old_status >= 400) {
...
void xslt_transform(xmlDocPtr doc, const char *xslfilename : itype(_Ptr<const char> ) , _Ptr<client_t> client, int status, _Nt_array_ptr<const char> location)
{
    xmlDocPtr res;
