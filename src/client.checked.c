...
static void client_send_500(_Ptr<client_t> client, const char *message);
...
static void _client_send_report(_Ptr<client_t> client, _Ptr<const char> uuid, _Ptr<const char> message, int http_status, _Ptr<const char> location)
{
    _Ptr<reportxml_t> report = NULL;
...
void client_send_reportxml(_Ptr<client_t> client, _Ptr<reportxml_t> report, document_domain_t domain, _Nt_array_ptr<const char> xsl, admin_format_t admin_format_hint, int status, _Nt_array_ptr<const char> location)
{
    ...
    if (admin_format == ADMIN_FORMAT_RAW) {
        ...
        _Nt_array_ptr<const char> document_domain_path = NULL;
...
{
   _Ptr<reportxml_node_t> rootnode = NULL;
_Ptr<reportxml_node_t> extension = NULL;
...
reportxml_t * client_get_reportxml(_Ptr<const char> state_definition, _Ptr<const char> state_akindof, _Ptr<const char> state_text)
{
    ...
    if (!report) {
       _Ptr<reportxml_node_t> rootnode = NULL;
_Ptr<reportxml_node_t> incidentnode = NULL;
_Ptr<reportxml_node_t> statenode = NULL;
        ...
        if (state_text) {
            _Ptr<reportxml_node_t> textnode = NULL;
